...
function isAuthorized(request, env) {
  ...
  const expected = env.ADMIN_PIN || env.APP_PIN || '';
...
async function insertSubtasks(env, taskId, subtasks = []) {
  ...
  for (const item of subtasks || []) {
    ...
    const subtaskId = `${taskId}_${slugify(rawId)}`;

    if (!subtaskId || subtaskId === `${taskId}_`) continue;
    ...
    await env.DB.prepare(`
      INSERT OR IGNORE INTO subtasks (id, name_en, name_de, weight, sort_order)
      VALUES (?, ?, ?, ?, ?)
    `)
...
export async function onRequestPost({ request, env }) {
  if (!isAuthorized(request, env)) {
    return json({ error: 'Admin PIN is missing or incorrect.' }, 401);
  }
