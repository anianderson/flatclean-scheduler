...
const translations = {
  en: {
    ...
    assignedTo: person => `Assigned to ${person}`,
    changedFromTo: (from, to) => `Changed from ${from} to ${to}.`,
    loadMore: 'Load more',
  ...
  de: {
    ...
    assignedTo: person => `Zugewiesen an ${person}`,
    changedFromTo: (from, to) => `Geändert von ${from} zu ${to}.`,
    loadMore: 'Mehr laden',
...
function App() {
  ...
  useEffect(() => {
    if (activePage === 'assignments' && !assignmentLogs.length && !assignmentLoading) {
      loadAssignmentLogs(true);
    }
  ...
  async function loadAssignmentLogs(reset = false) {
    try {
      ...
      const cursorQuery = !reset && assignmentCursor
        ? `&cursor=${encodeURIComponent(assignmentCursor)}`
        : '';

      const res = await fetch(`/api/assignment-log?limit=20${cursorQuery}`);
      const json = await res.json().catch(() => ({}));
  ...
  const navItems = [
    ...
    { id: 'assignments', label: t.assignmentLog, icon: Sparkles },
    ...
    return (
      <section className="card assignment-log-page">
        ...
        <div className="assignment-log-list">
          ...
          {!assignmentLogs.length && !assignmentLoading && (
            <div className="empty-state">{t.noAssignmentLogs}</div>
          )}
          ...
          {assignmentCursor && (
            <button
              ...
              onClick={() => loadAssignmentLogs(false)}
            >
              {assignmentLoading ? t.processing : t.loadMore}
            </button>
          )}
        </div>
      </section>
  ...
  return (
    <>
      ...
      <main className="page" id="top">
        ...
        {activePage === 'assignments' && renderAssignmentLogPage()}
