...
async function assertKnownFlatmate(env, person) {
...
export async function onRequestPost({ request, env }) {
  ...
  const isAdminAction = body.admin === true || String(body.admin || '').toLowerCase() === 'true';
  ...
  if (!(await assertKnownFlatmate(env, person))) {
  ...
  if (action === 'delete') {
    ...
    if (!isAdminAction && normalizeName(existing.person) !== person) {
  ...
  if (action !== 'add') {
    return json({ error: 'Unknown absence action.' }, 400);
  }
